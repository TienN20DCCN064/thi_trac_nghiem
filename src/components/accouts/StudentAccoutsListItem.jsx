...
const studentAccoutActions = createActions("studentAccout");

const StudentAccoutsListItem = ({ data = [], onDataChange }) => {
  ...
  // Submit
  const handleSubmit = () => {
    if (!formData.id_tai_khoan || !formData.ma_sv) {
      message.error("Vui lòng nhập đầy đủ ID tài khoản và mã sinh viên!");
      return;
    }
    ...
    if (modalMode === "create") {
      dispatch(
        studentAccoutActions.creators.createRequest(payload, (res) => {
          if (res.success) {
            message.success(res.message || "Thêm tài khoản thành công!");
            setModalVisible(false);
            setFormData({ id_tai_khoan: "", ma_sv: "" });
            onDataChange();
          } else {
      ...
      dispatch(
        studentAccoutActions.creators.updateRequest(
  ...
  const handleDelete = (record) => {
    dispatch(
      studentAccoutActions.creators.deleteRequest(record.id_tai_khoan, (res) => {
        if (res.success) {
      ...
      >
        {modalMode === "view" && (
          <div>
            ...
            <p>
              <b>Mã sinh viên:</b> {selectedRecord?.ma_sv}
            </p>
          </div>
        )}
...
export default StudentAccoutsListItem;